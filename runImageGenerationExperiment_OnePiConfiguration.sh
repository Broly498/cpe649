...
sudo taskset 0x8 ./$POWER_MEASUREMENT_DIRECTORY/profiler -p -t 10 > $PI_HAT_POWER_MEASUREMENTS_OUTPUT_DIRECTORY/${current_time}_piHatResuls.csv & sleep $PROFILER_IDLE_TIME_s
taskset 0x1 python3 $IMAGE_GENERATION_DIRECTORY/networked-encrypt-image-data.py $ENCRYPTION_DATA_CLIENT_IP_ADDRESS $1 $2 &
taskset 0x2 python3 $IMAGE_GENERATION_DIRECTORY/networked-compress-image-data.py $COMPRESSION_DATA_CLIENT_IP_ADDRESS &
